...
export async function drawMermaid(container: HTMLElement, current: () => boolean): Promise<void> {
  ...
  for (const element of elements) {
    ...
    try {
      ...
      element.innerHTML = `<div class="diagram-actions"><button type="button" data-diagram-action="source">ソース</button><button type="button" data-diagram-action="expand">拡大表示</button></div><div class="diagram-image">${svg}</div>`;
      element.dataset.rendered = 'true';
    } catch (error) {
      if (!current() || !element.isConnected) return;
      element.replaceChildren();
      const heading = document.createElement('strong'); heading.textContent = '図を表示できません';
      const message = document.createElement('p'); message.className = 'diagram-error'; message.textContent = String(error).replace(/^Error:\s*/, '');
      const details = document.createElement('details'); const summary = document.createElement('summary'); summary.textContent = 'ソースを表示';
