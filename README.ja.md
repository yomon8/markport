...
クリップボードの文章を確認するには、**Paste Markdown** を押して入力欄に貼り付け、**Render** を押します。通常の Markdown 機能を使えます。貼り付けた文章には基準フォルダがないため、相対リンクと画像は文字として表示します。文章は **Clear** を押すかタブを閉じるまで、再読み込み後も同じタブに残ります。表示のため Markport サーバーへ送信しますが、ファイルには保存しません。貼り付けられる Markdown は 1 MiB までです。
...
現行版に編集、コメント、本文全文検索、標準入力からの取り込み、認証付きのネットワーク公開は含めません。
