...
func TestFinalSwapWithLinkAndFIFO(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(dir, "target.md"), []byte("safe"), 0644); err != nil {
		t.Fatal(err)
	}
	s, err := New(dir)
	if err != nil {
	...
	case err := <-result:
		if err != nil {
			t.Fatal("read link target")
		}
	case <-time.After(3 * time.Second):
		t.Fatal("read blocked on swapped FIFO")
	}
	close(stop)
	<-done
}
